...
реализует интеграцию с сервисом **Google ReCaptcha v3**. В отличие от старых версий, v3 работает незаметно для пользователя, возвращая «оценку» (score) от 0.0 до 1.0, где 1.0 — это гарантированный человек, а 0.0 — бот.
...
Токен должен передаваться в параметре запроса `g-recaptcha-response`. Если ваш фронтенд использует другой заголовок (например, `X-ReCaptcha-Token`), его нужно извлечь и передать в метод `validateToken` вручную.