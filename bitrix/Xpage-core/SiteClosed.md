...
Этот класс отвечает за функционал **«Заглушки» (Site Maintenance mode)**. Он позволяет закрыть публичную часть сайта от обычных посетителей, но при этом оставить доступ для разработчиков, администраторов и доверенных лиц.


## 1. Основная логика: метод `userHasAccess()`
...
## 2. Подробный разбор механизмов доступа
...
### 3. Как происходит блокировка (`showClosedPage`)
