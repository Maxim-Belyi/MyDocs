...
В Битриксе "Событие" (Event) в контексте базы данных — это почти всегда **почтовое событие**. Вся система отправки писем строится на трех таблицах.
...
*   **LID (Language ID / Site ID):**
    Символьный код сайта (например, `s1`), к которому относится событие.
*   **C_FIELDS:**
...
*   **DESCRIPTION:**
    ...
    *   *Пример:* `#USER_ID# - ID пользователя`, `#EMAIL# - Email пользователя`. Это подсказка для администратора при создании шаблона.
...
*   **LID:**
    Привязка к сайту.
*   **EMAIL_FROM / EMAIL_TO:**
    От кого и кому. Здесь могут быть макросы (например, `#EMAIL_TO#`).
*   **SUBJECT:**
    Тема письма.
*   **MESSAGE:**
...
Битрикс не хранит файлы в базе (BLOB), он хранит только ссылки на них.
...
*   **HEIGHT / WIDTH:**
    Высота и ширина (если это изображение). Позволяет узнать размеры, не открывая сам файл с диска.
*   **FILE_SIZE:**
    Размер в байтах.
*   **CONTENT_TYPE:**
