...
Этот раздел описывает механизм **перехвата событий (хуков)**, который позволяет изменять логику работы ядра или модулей Битрикса, не редактируя их исходный код.
...
Битрикс разделяет события на две большие группы по времени их срабатывания:
...
#### Способ 1: В файле `init.php` (Статическое подключение)
Это самый частый способ для кастомизации конкретного проекта. Код пишется в `/local/php_interface/init.php`. (чтобы не было лапши из кода советую вызвать классы)
...
#### Способ 2: Через D7 (Современный подход)
Использует менеджер событий `EventManager`. Более гибкий и рекомендуется для новых проектов.
...
#### Пример 2: Автоматическое создание символьного кода
Если контент-менеджер забыл заполнить поле "Символьный код" (CODE), генерируем его из названия.

```php
EventManager::getInstance()->addEventHandler(
    "iblock", "OnBeforeIBlockElementAdd", "generateElementCode"
);

function generateElementCode(&$arFields) {
...
### 4. Старое ядро vs Новое ядро (D7)
...
1.  **Чистота обновлений:** Вы можете обновлять Битрикс до последней версии, ваши изменения не затрутся (в отличие от редактирования файлов в `/bitrix/modules/`).
2.  **Централизация:** Вся бизнес-логика проекта (проверки, уведомления) собрана в одном месте (`init.php` или отдельном модуле).
3.  **Гибкость:** Можно навесить неограниченное количество «слушателей» на одно и то же действие.

### Минусы:
1.  **Сложность отладки:** Иногда трудно найти, какой именно обработчик изменил данные (помогает поиск по коду `addEventHandler`).
2.  **Зацикливание:** Если внутри обработчика `OnAfterIBlockElementUpdate` вызвать метод `Update()` для того же элемента, возникнет бесконечный цикл. Нужно использовать флаги для предотвращения повторного входа.
